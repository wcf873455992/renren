$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'renrenapi/garden/list',
        datatype: "json",
        colModel: [			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.garden.gardenId == null ? "renrenapi/garden/save" : "renrenapi/garden/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "renrenapi/garden/delete",
                        contentType: "application/json",
		...
		getInfo: function(gardenId){
			$.get(baseURL + "renrenapi/garden/info/"+gardenId, function(r){
                vm.garden = r.garden;
            });
		},
