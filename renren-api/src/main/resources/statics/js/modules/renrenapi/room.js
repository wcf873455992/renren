$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'renrenapi/room/list',
        datatype: "json",
        colModel: [			
			...
			{ label: '所属园区', name: 'gardenId', index: 'garden_id', width: 80 }, 			
	...
	methods: {
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.room.roomId == null ? "renrenapi/room/save" : "renrenapi/room/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "renrenapi/room/delete",
                        contentType: "application/json",
		...
		getInfo: function(roomId){
			$.get(baseURL + "renrenapi/room/info/"+roomId, function(r){
                vm.room = r.room;
            });
		},
